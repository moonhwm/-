...
> 版本 v1.0 · 2026-08-28 · 本文件随 `gemma4-deploy/` 目录走
...
| **GPU 14 层（BF16 历史档）** | **8.7 s** | **1.7–1.8 s（≈18 tok/s）** | **约 25×** |
| **Q4 全载 43 层（生产现用，08-29 起）** | — | **≈47 tok/s** | **约 66×** |
...
├── watchdog.py         # 看门狗：120s 探活，连续 2 次失败自动拉起
...
## 8. 双档位运营（2026-08-29 03:07 起，死机事件后的策略调整）

背景：01:07 系统死机被动重启——16 GB 常驻 15 GB BF16 的首日警示风险兑现。改为双档位：

| 档位 | model_file | n_gpu_layers | 实测 | 用途 |
|---|---|---|---|---|
| **日常档（当前）** | `gemma-4-E4B-it-Q4_K_M.gguf`（4.97 GB） | `-1`（43 层全上卡） | **47.5 tok/s**（101 tok/2.1s），显存 5.0 GB，零换页 | 常驻，手机/平板随时用 |
| 质量档 | `gemma-4-E4B-it-BF16.gguf`（15 GB） | `14` | 热态 ≈18 tok/s，冷态可 <1 tok/s | 追求质量时临时切 |

**切档方法**：改 `server.config.json` 两个键 → `schtasks /run /tn Gemma4-BF16-Local`（或等看门狗自然接管）。注意：质量档在 16 GB 内存机器上有系统卡死风险（已发生一次），用后建议切回日常档。

**注意**：`n_gpu_layers: -1` 曾导致 `_maybe_preload_cuda` 跳过 DLL 预载（`<=0` 误判）→ llama.dll 依赖缺失崩溃，已修为 `==0` 才跳过。conf: empirical（03:13 实录）。
